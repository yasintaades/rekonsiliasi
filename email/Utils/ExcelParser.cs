                ...
                using (reader)
                {
                    ...
                    foreach (DataRow row in table.Rows.Cast<DataRow>().Skip(1))
                    {
                        ...
                        var refNo = GetValue(row, map,
                            "Order Number", "Internal ref", "Ref No", "internalReference", "Reference Number");

                        if (string.IsNullOrWhiteSpace(refNo))
                            continue;

                        // Mapping SKU
                        var sku = GetValue(row, map, "SKU", "Seller Sku", "Article", "Product Sku", "SKU Cegid", "SKU Anchanto");

                        // Mapping Tanggal
                        // 1. Ambil string mentah dari file
                        var dateStr = GetValue(row, map, 
                            "Date", "Gi_posting_date", "Gi_posting_date II", "Order Date","Completed Date", "Date Cegid");
                        ...
                        var qtyStr = GetValue(row, map, "Ordered Quantity", "Gi_qty", "Qty", "Stok", "Consignment Quantity","Stock Anchanto","Stock Cegid");
