...
namespace Reconciliation.Api.Services
{
    public class SftpWatcherService : BackgroundService
    {
        ...
        private readonly string _downloadFolder = @"C:\Users\user\delamibrands\Reconciliation.Api\bin\Debug\net8.0\Downloads";
        ...
        // Di dalam method download SftpWatcherService
public async Task DownloadFile(ISftpClient sftp, SftpFile remoteFile)
{
    // Gunakan BaseDirectory dan naik ke folder utama project


// Ganti bagian penentuan path di SftpWatcherService
var baseDir = AppDomain.CurrentDomain.BaseDirectory;
// Naik 3 level dari bin/Debug/net8.0 ke Root Project
...
{
    // Ini bagian krusial: Pastikan stream benar-benar terisi
    sftp.DownloadFile(remoteFile.FullName, fileStream);
    Console.WriteLine($"[SFTP SUCCESS] File disimpan ke: {localPath}");
}

    // 3. Download filenya
    using (var saveFileStream = System.IO.File.OpenWrite(localPath))
    {
        await Task.Run(() => sftp.DownloadFile(remoteFile.FullName, saveFileStream));
        Console.WriteLine($"[SFTP] Berhasil download ke: {localPath}");
    }
}
                        ...
                        using (var client = new SftpClient(connectionInfo))
                        {
                            ...
                            foreach (var file in files)
                            {
                                if (!file.IsDirectory && file.Name.EndsWith(".csv", StringComparison.OrdinalIgnoreCase))
                                {
                                    ...
                                    // 1. Download jika file belum ada di lokal
                                    ...
                                    if (!await repo.IsFileNameExists(file.Name))
                                    {
                                        await repo.SaveSyncLog(new FtpSyncLog {
                                            ...
                                            Status = "Ready"
                                            // CreatedAt = DateTime.Now // Tambahkan jika model FtpSyncLog punya property ini
                                        });
