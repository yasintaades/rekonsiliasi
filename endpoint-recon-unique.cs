...
public static class ReconciliationEndpoint
{
    public static void MapReconciliationEndpoints(this WebApplication app)
    {
        ...
        app.MapPost("/reconciliations/upload", async (HttpRequest http, IConfiguration config) =>
        {
            ...
            var anchantoRefs = ReadExcel(files[0]);
var cegidRefs = ReadExcel(files[1]);

var anchantoSet = new HashSet<string>(anchantoRefs);
var cegidSet = new HashSet<string>(cegidRefs);

var details = new List<ReconciliationDetail>();

// 🔹 MATCH + ONLY ANCHANTO
foreach (var a in anchantoSet)
{
    if (cegidSet.Contains(a))
    {
...
// 🔹 ONLY CEGID
foreach (var c in cegidSet)
{
    if (!anchantoSet.Contains(c))
    {
        details.Add(new ReconciliationDetail
        {
            RefNo = c,
            ...
            var summary = new
            {
                totalAnchanto = anchantoSet.Count,
                totalCegid = cegidSet.Count,
    ...
    // 🔹 Read Excel → List, keep duplicates
    // ==========================
    private static List<string> ReadExcel(IFormFile file)
{
    ...
    {
        var refNo = sheet.Cells[row, 1].Value?.ToString()?.Trim().ToUpper();
