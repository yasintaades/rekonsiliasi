 ...
 const handleDownload = async () => {
  ...
  try {
    const res = await fetch(
      `http://localhost:5000/reconciliationsPO/download/${result.reconciliationId}`
    );
            ...
            <table className="w-full text-sm border">
              ...
              <tbody>
                ...
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <td className="p-2 border">{d.amount1 ?? "-"}</td> {/* amount1 kecil */}
                  <td className="p-2 border">{formatDate(d.date1)}</td>  {/* date1 kecil */}
