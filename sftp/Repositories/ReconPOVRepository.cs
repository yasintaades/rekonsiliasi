...
namespace Reconciliation.Api.Repositories
{
    public class ReconPOVRepository
    {
        ...
        public async Task<int> Save(List<ReconciliationDetail3> details)
        {
            ...
            cmdHeader.Parameters.AddWithValue("f", "PO VIRTUAL_" + DateTime.Now.ToString("yyyyMMdd"));
            ...
            var sql = @"INSERT INTO reconciliation_details_3 (
                ...
                sender_site_received, receive_site_received,item_name_received, sku_received, date_received, qty_received, unit_cogs_received,
                status
            ) VALUES (
                @rid, @ref,@ss,@rs,@it,@uc,@stn,@dtn,@qtn,@cn,@sc,@dc,@qc,@ssr,@rsr,
                @ir,@sr,@dr,@qr,@ucr,@stat
            )
            ON CONFLICT (
                ref_no,
                COALESCE (sku_transfer_notice, ''),
                COALESCE (sku_consignment, ''),
                COALESCE (sku_received, '')
            )
            DO UPDATE SET
                sender_site = EXCLUDED.sender_site,
                receive_site = EXCLUDED.receive_site,
                item_name_transfer = EXCLUDED.item_name_transfer,
                unit_cogs = EXCLUDED.unit_cogs,
                qty_transfer_notice = EXCLUDED.qty_transfer_notice,
                date_transfer_notice = EXCLUDED.date_transfer_notice,
                consignment_no = EXCLUDED.consignment_no,
                qty_consignment = EXCLUDED.qty_consignment,
                date_consignment = EXCLUDED.date_consignment,
                sender_site_received = EXCLUDED.sender_site_received,
                receive_site_received = EXCLUDED.receive_site_received,
                item_name_received = EXCLUDED.item_name_received,
                qty_received = EXCLUDED.qty_received,
                date_received = EXCLUDED.date_received,
                unit_cogs_received = EXCLUDED.unit_cogs_received,
        ...
        public async Task<List<ReconciliationDetail3>> GetAllForMatching()
        {
            using var conn = new NpgsqlConnection(_config.GetConnectionString("Default"));

            var sql = @"SELECT 
                ref_no AS RefNo,
                sku_transfer_notice AS SkuTransfer,
                sku_consignment AS SkuConsignment,
                sku_received AS SkuReceived,
                qty_transfer_notice AS QtyTransfer,
                qty_consignment AS QtyConsignment,
                qty_received AS QtyReceived,
                date_transfer_notice AS DateTransfer,
                date_consignment AS DateConsignment,
                date_received AS DateReceived,
                status AS Status
            FROM reconciliation_details_3";

            var result = await conn.QueryAsync<ReconciliationDetail3>(sql);
        ...
        {
            using var conn = new NpgsqlConnection(_config.GetConnectionString("Default"));
            var sql = "SELECT id, file_name, status FROM ftp_sync_logs WHERE id = @id";
            return await conn.QueryFirstOrDefaultAsync<FtpSyncLog>(sql, new { id = logId });
        }

        public async Task MarkAsDone(int logId)
        {
            using var conn = new NpgsqlConnection(_config.GetConnectionString("Default"));
            var sql = "UPDATE ftp_sync_logs SET status = 'DONE' WHERE id = @id";
            await conn.ExecuteAsync(sql, new { id = logId });
        }
    }
}