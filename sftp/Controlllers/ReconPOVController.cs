...
namespace Reconciliation.Api.Controllers
{
    ...
    public class ReconPOVController : ControllerBase
    {
        ...
        public async Task<IActionResult> ProcessMixed(
            [FromForm] ReconPOVMixedRequest request, // Mengambil dari body (FormData)
            [FromRoute] int logId2,          // Mengambil dari URL
            [FromRoute] int logId3           // Mengambil dari URL
        )
        {
            if (request.ManualFile == null) return BadRequest("File manual belum dipilih.");
            
            var result = await _service.ProcessTripleMixed(request.ManualFile, logId2, logId3);
            return Ok(result);
        }
